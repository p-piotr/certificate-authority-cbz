...
#define ENCRYPTED_PRIVATE_KEY 2
#define CSR 3
#define CERTIFICATE 4
void write_pkcs_to_file(const ASN1Object& root_object, int type, std::string filepath) {
...
int main(int argc, char* argv[]){
    ...
    write_pkcs_to_file(ca_certificate.to_asn1(), CERTIFICATE, ca_certificate_filepath);
    std::cout << "CA certificate written to " << ca_certificate_filepath << std::endl;
    std::cout << "Signature verification: "
        << std::boolalpha << ca_certificate.verify(ca_certificate) << std::noboolalpha << std::endl;
    // verify yourself with `openssl verify -CAfile cacert.pem cacert.pem`

    std::cout << "Creating entity certificate..." << '\n';
    ...
    write_pkcs_to_file(subject_csr.to_asn1(), CSR, subject_csr_filepath);
    std::cout << "CA certificate written to " << subject_csr_filepath << std::endl;
    std::cout << "Signature verification: "
        << std::boolalpha << subject_csr.verify() << std::noboolalpha << std::endl;
    // verify yourself with `openssl req -in subcsr.pem -noout -verify`

    // create the subject CA
    // TODO: create the subject cert based on CSR and CA cert
    // 1. implement CertificationRequest::from_file constructor
    // 2. implement Certificate::from_file constructor
    // 3. use those two to generate certificate using generate_certificate function above
    // 4. fix arbitrary values used in generate_self_signed_certificate and generate_certificate
    // 5. move write_pkcs_to_file to a more sensible place (like utils/io.cpp)
