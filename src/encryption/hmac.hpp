    ...
    public:
        ...
        static void derive_blocksized_key(
            ...
            if (ik.size() < KEY_SIZE) {
                ...
                CBZ::Security::secure_zero_memory(dk + ik.size(), KEY_SIZE - ik.size());
                return;
            }

            // ik.size() > KEY_SIZE
            _H::digest(ik, dk);
            CBZ::Security::secure_zero_memory(dk + DIGEST_SIZE, KEY_SIZE - DIGEST_SIZE);
        }
        ...
        static void digest(
            ...
            uint8_t dk[KEY_SIZE];
            uint8_t dk_opad[KEY_SIZE];
            uint8_t dk_ipad[KEY_SIZE];
            uint8_t ihash[DIGEST_SIZE];
            ...
            CBZ::Security::secure_zero_memory(dk);
            CBZ::Security::secure_zero_memory(dk_ipad);
            ...
            CBZ::Security::secure_zero_memory(dk_opad);
            CBZ::Security::secure_zero_memory(ihash);
            CBZ::Security::secure_zero_memory(concat_message);
        }
    };
}