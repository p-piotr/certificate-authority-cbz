...
std::vector<uint8_t> EMSA_PKCS1_V1_5_ENCODE_sha256(std::vector<uint8_t> const& M, size_t emLen=256){
    ...
    std::shared_ptr<std::vector<uint8_t>> dig_os = ASN1Parser::encode(ASN1Object(OCTET_STRING, std::move(dig)));
    std::vector<uint8_t> digestInfo = encode_der_sequence({digestAlgorithm.encode(), digest});
...
std::vector<uint8_t> RSASSA_PKCS1_V1_5_SIGN(RSAPrivateKey const& K, std::vector<uint8_t> const& M){
